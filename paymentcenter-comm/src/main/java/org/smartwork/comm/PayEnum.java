...
public enum PayEnum {
    ...
    */
    ...
    public static List<ResultEnum> resultEnums(){
        return Arrays.asList(PayEnum.values())
                .stream().map(payEnum -> ResultEnum.ResultEnumBuild
                        .build()
                        .setCode(payEnum.getCode())
                        .setName(payEnum.getMessage())).collect(Collectors.toList());
    }
    ...
    public static boolean existsCode(Object code){
        return Arrays.asList(PayEnum.values()).stream()
                .filter(payEnum -> ConvertUtils.isNotEmpty(code)&&payEnum.getCode().equals(code))
                .count() >=  1;
    }
