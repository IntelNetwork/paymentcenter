...
public enum RetEnum {
    ...
    public static List<ResultEnum> resultEnums(){
        return Arrays.asList(RetEnum.values())
                .stream().map(retEnum -> ResultEnum.ResultEnumBuild
                        .build()
                        .setCode(retEnum.getCode())
                        .setName(retEnum.getMessage())).collect(Collectors.toList());
    }
    ...
    public static boolean existsCode(Object code){
        return Arrays.asList(RetEnum.values()).stream()
                .filter(retEnum -> ConvertUtils.isNotEmpty(code)&&retEnum.getCode().equals(code))
                .count() >=  1;
    }
